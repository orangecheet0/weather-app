...
import { DailyBlock, Unit } from "@/types";
import { useWeatherFormatting } from "@/hooks/useWeatherFormatting";

export default function DailyForecast({ data, unit }: { data: DailyBlock; unit: Unit }) {
  const { shortDate, weatherIcon, formatTemp } = useWeatherFormatting();
