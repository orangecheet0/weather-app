...
import { HourlyBlock, Unit } from "@/types";
import { useWeatherFormatting } from "@/hooks/useWeatherFormatting";

export default function HourlyForecast({ data, unit }: { data: HourlyBlock; unit: Unit }) {
  const { shortTime, weatherIcon, formatTemp } = useWeatherFormatting();
  const now = new Date();
